...
export default function HomeContentEditor() {
  ...
  useEffect(() => {
    async function loadData() {
      const data = await getDocument<HomeContent>('siteContent', 'home');
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    setSaving(true);
    await setDocument('siteContent', 'home', content);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
      <div className="border-b border-gray-200 pb-4">
        ...
        <p className="text-sm text-gray-500 mt-1">Manage the content and SEO tags for the public home page.</p>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-8">
        ...
        <div className="bg-white p-6 shadow-sm border border-gray-200 rounded-lg space-y-4">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea value={content.hero.subheading} onChange={(e) => setContent({...content, hero: {...content.hero, subheading: e.target.value}})} className="mt-1 block w-full border border-gray-300 p-2 focus:ring-black focus:border-black rounded-md sm:text-sm" rows={3}></textarea>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700">CTA Text</label>
                <input type="text" value={content.hero.ctaText} onChange={(e) => setContent({...content, hero: {...content.hero, ctaText: e.target.value}})} className="mt-1 block w-full border border-gray-300 p-2 focus:ring-black focus:border-black rounded-md sm:text-sm" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">CTA Link</label>
                <input type="text" value={content.hero.ctaLink} onChange={(e) => setContent({...content, hero: {...content.hero, ctaLink: e.target.value}})} className="mt-1 block w-full border border-gray-300 p-2 focus:ring-black focus:border-black rounded-md sm:text-sm" />
              </div>
            </div>
          </div>
        </div>
        ...
        {/* INTRO SECTION */}
        <div className="bg-white p-6 shadow-sm border border-gray-200 rounded-lg space-y-4">
          <h2 className="text-lg font-semibold border-b border-gray-200 pb-2">Intro Section</h2>
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700">Heading</label>
